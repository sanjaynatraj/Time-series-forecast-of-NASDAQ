...
for (files in filename) {
  ...
  if(file.info(files)[1]>0) {
    ...
    #fitData <- lm(textData$Adj.Close~textData$Date,data=trainData)
    fitData <- tslm(trainData~trend,lambda = NULL)
...
jpeg(filename = "lm.jpeg", width = 1081, height = 664, units = "px", pointsize = 12, quality = 75,bg = "white")

plot(min_vals[1:10,2], main="Linear model", ylab="MAE", xlab="Stock_Name")
